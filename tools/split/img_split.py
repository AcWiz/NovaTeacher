...
def add_parser(parser):
    ...
    parser.add_argument(
        '--base-json',
        type=str,
        default='data/tools/split/split_configs/ss_train.json',
...
def parse_args():
    ...
    assert args.save_ext in ['.png', '.jpg', 'bmp', '.tif', '.fits']
...
#     """Modified version to handle FITS files while preserving precision.
...
#         img_ext (str): Picture suffix ('.fits' or regular image extensions).

#     Returns:
#         list[dict]: Information of paths.
#     """
#     img_path = osp.join(img_dir, info['filename'])
...
#             if k not in ['id', 'filename', 'width', 'height', 'ann']:
#                 patch_info[k] = v
...
#         # Process and save based on file type
#         if img_path.lower().endswith('.fits'):
#             # Create new HDUList for the cropped region
#             new_hdul = fits.HDUList()
            
#             for idx, hdu_data in enumerate(hdus_data):
#                 if hdu_data['data'] is not None:
#                     # Handle different dimensionality
#                     if hdu_data['data'].ndim == 2:
#                         # For 2D data (single image)
#                         print('For 2D data (single image)\n')
#                         cropped_data = hdu_data['data'][y_start:y_stop, x_start:x_stop]
#                     elif hdu_data['data'].ndim == 3:
#                         # For 3D data (multiple frames or color channels)
#                         cropped_data = hdu_data['data'][:, y_start:y_stop, x_start:x_stop]
#                     elif hdu_data['data'].ndim == 4:
#                         # For 4D data
#                         cropped_data = hdu_data['data'][:, :, y_start:y_stop, x_start:x_stop]
#                     else:
#                         # Skip HDUs with unexpected dimensions
#                         continue
...
#                         if height > cropped_data.shape[-2] or width > cropped_data.shape[-1]:
#                             # Create padding array with same dtype as original
#                             pad_shape = list(cropped_data.shape)
#                             pad_shape[-2] = height
#                             pad_shape[-1] = width
#                             padding_data = np.full(pad_shape, padding_value, dtype=cropped_data.dtype)
                            
#                             # Copy data into padded array
#                             if cropped_data.ndim == 2:
#                                 padding_data[:cropped_data.shape[0], :cropped_data.shape[1]] = cropped_data
#                             elif cropped_data.ndim == 3:
#                                 padding_data[:, :cropped_data.shape[1], :cropped_data.shape[2]] = cropped_data
#                             elif cropped_data.ndim == 4:
#                                 padding_data[:, :, :cropped_data.shape[2], :cropped_data.shape[3]] = cropped_data
                            
#                             cropped_data = padding_data

#                     # Create new HDU with cropped data
#                     new_hdu = hdu_data['type'](data=cropped_data)
                    
#                     # Copy and update header
#                     new_hdu.header = hdu_data['header'].copy()
                    
#                     # Update WCS information if present
#                     if 'CRPIX1' in new_hdu.header:
#                         new_hdu.header['CRPIX1'] -= x_start
#                     if 'CRPIX2' in new_hdu.header:
#                         new_hdu.header['CRPIX2'] -= y_start
                    
#                     # Add cropping information to header
#                     new_hdu.header['XSTART'] = x_start
#                     new_hdu.header['YSTART'] = y_start
#                     new_hdu.header['CUTOUT'] = 'True'
                    
#                 else:
#                     # For HDUs without data (e.g., headers only)
#                     new_hdu = hdu_data['type'](header=hdu_data['header'])
                
#                 new_hdul.append(new_hdu)

#             # Save cropped FITS file
#             output_path = osp.join(save_dir, patch_info['id'] + img_ext)
#             new_hdul.writeto(output_path, overwrite=True)
#             new_hdul.close()
            
#             # Update patch info dimensions from primary HDU
#             patch_info['height'] = new_hdul[0].data.shape[-2]
#             patch_info['width'] = new_hdul[0].data.shape[-1]
            
#         else:
#             # Handle regular image files (same as original)
...
def crop_and_save_img(info, windows, window_anns, img_dir, no_padding,
                       padding_value, save_dir, anno_dir, img_ext='.fits'):
    """
    Args:
        info (dict): Image's information.
        windows (np.array): information of sliding windows.
        ...
        img_dir (str): Path of images.
        no_padding (bool): If True, no padding.
        padding_value (int or float): Padding value.
        save_dir (str): Save filename.
        anno_dir (str): Annotation filename.
    ...
    # Use fits.getdata to read .fits image data directly
    img_data, img_header = fits.getdata(osp.join(img_dir, info['filename']), header=True)
    ...
    for i in range(windows.shape[0]):
        ...
        # Extract the patch
        patch = img_data[y_start:y_stop, x_start:x_stop]
        
        # Apply padding if needed
        if not no_padding:
            height, width = y_stop - y_start, x_stop - x_start
            if height > patch.shape[0] or width > patch.shape[1]:
                padding_patch = np.full((height, width), padding_value, dtype=patch.dtype)
                padding_patch[:patch.shape[0], :patch.shape[1]] = patch
                patch = padding_patch
        ...
        # Save the patch as a .fits file
        patch_filename = osp.join(save_dir, patch_info['id'] + img_ext)
        fits.writeto(patch_filename, patch, header=img_header, overwrite=True)
        ...
        bboxes_num = patch_info['ann']['bboxes'].shape[0]
        outdir = osp.join(anno_dir, patch_info['id'] + '.txt')
...
def _load_dota_single(imgfile, img_dir, ann_dir):
    ...
    if ext not in ['.jpg', '.JPG', '.png', '.tif', '.bmp', '.fits']:
        return None
